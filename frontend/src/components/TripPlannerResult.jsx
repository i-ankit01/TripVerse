...
export function TripPlannerResult({ data, isLoading }) {
      ...
      <Card className="shadow-lg border-0 bg-card/80 backdrop-blur-sm">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted-foreground text-center text-sm">
            Our AI is analyzing destinations, finding the best deals, and
    ...
    return (
      <Card className="shadow-lg border-0 bg-card/80 backdrop-blur-sm">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted-foreground text-center text-sm">
            Fill out the form to get your personalized AI-generated trip plan
            with costs, itinerary, and recommendations.
          </p>
        </CardContent>
      </Card>
    );
  ...
  const { local, ...main } = data.transportation;
  const mainMode = Object.keys(main)[0]; // to get the dynamic mode of trasport that user selected
        ...
        <CardContent>
          <div className="text-center">
            <div className="text-3xl font-bold text-primary mb-1">
              ₹{data.totalCost.min.toLocaleString()} - ₹
              {data.totalCost.max.toLocaleString()}
            </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="flex items-center gap-1">
                ...
                {mainMode.charAt(0).toUpperCase() + mainMode.slice(1)}
              </span>
              <span>{main[mainMode]?.estimated}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="flex items-center gap-1">
